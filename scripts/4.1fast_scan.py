...
async def probe_one(url, sem, timeout, max_retries=2):
    """
    对一个url进行probe，失败时重试，最多尝试max_retries+1次
    """
    attempt = 0
    async with sem:
        while attempt <= max_retries:
            ...
            if "probe" in res:
                ...
                if attempt > max_retries:
                    # 重试次数用尽，返回失败结果
                    return {
                        "url": url, "has_video": False, "has_audio": False,
                        "video_codec": None, "width": None, "height": None, "frame_rate": None,
                        "duration": None, "bit_rate": None, "error": error_msg, "retries": attempt
                    }
                # 重试前可以短暂等待（可选），避免频繁调用
...
async def run_all(urls, concurrency=10, timeout=20):
    ...
    total = len(tasks)
    # tqdm 只在每1%时打印日志
    log_interval = max(total // 100, 1)
    completed_count = 0
    for fut in tqdm_asyncio.as_completed(tasks, total=total, desc="deep-scan"):
        r = await fut
        results.append(r)
        completed_count += 1
        if completed_count % log_interval == 0:
            print(f"Progress: {completed_count}/{total} ({completed_count*100/total:.1f}%)")
    return results
...
def write_out(results, outpath=OUTPUT):
    fieldnames = ["url","has_video","has_audio","video_codec","width","height","frame_rate","duration","bit_rate","error","retries","视频信息"]
    with open(outpath, "w", newline='', encoding='utf-8') as f:
        ...
        for r in results:
            ...
            row = {k: r.get(k, "") for k in fieldnames[:-1]}  # 先复制已有字段
...
def main():
    ...
    p.add_argument("--concurrency", type=int, default=30)  # 默认30并发
